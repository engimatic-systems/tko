...
lint_note_titles(path, text):
  ...
  inspect level-3 note headings until the next level-1 or level-2 heading
...
#+begin_example
org_heading(line):
  parse leading stars followed by one space as an Org heading
  return heading level and trimmed title

semantic_heading(title):
  normalize known semantic heading names to canonical display text
