...
sections must hold non-whitespace content. =close= can fill the first required
section from =--reason= before the gate runs; =set_status= (and therefore
...
set_status(store, id, status):
  ...
  when status is closed, require the type's required-at-close sections non-empty
...
close(store, id, reason):
  resolve partial id to a ticket stem
  when reason is given, write it into the type's first required-at-close section
...
check_close_requirements(store, resolved):
  ...
  fail on the first required-at-close section without non-whitespace content

write_close_reason(store, resolved, reason):
  refuse when the type has no required-at-close section
  expand escaped newlines in the reason
  write the reason into the first required-at-close section on disk

section_has_content(body, heading):
  scan the level-2 section under the case-insensitive heading
  content is any non-whitespace line before the next heading of level <= 2
