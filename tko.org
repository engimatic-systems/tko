...
Both rules consume the storage type shape table; the ticket type comes from
=TKO_TYPE= in the property drawer (default =task=). Files whose drawer does not
parse, or whose type has no shape row, skip these rules; the structural rules
above still run.
...
lint_type_shape(path, text):
  parse the ticket; skip typed rules on parse failure or unknown type
