...
#+begin_example
NoteEntry:
  stores the printable heading text
  stores title text without timestamp
  stores optional timestamp text
  stores the full *** note subtree for show --note
#+end_example
...
split_lines(text):
  preserve line endings for subtree reconstruction
