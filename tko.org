...
append_note(document, note):
  if ** Notes exists, insert before the next top-level or second-level heading
  otherwise append a ** Notes section to the document
  preserve existing line endings enough to avoid joining headings and notes
#+end_example
