...
semantic heading. A flag given for a type whose allowed headings do not include
that section is refused. Sections the type requires at create must be given
non-empty content. Description is allowed on every type.

#+begin_example
validate_section_inputs(input, shape):
  refuse any given section flag whose heading is not allowed for the type
  refuse any required-at-create heading whose input is missing or blank
