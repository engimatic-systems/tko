...
- =scaffold= lists type-specific headings emitted at create, in document order.
...
create(store, cwd, input):
  ...
  write .tickets/<id>.org with TKO_* properties and the normalized sections
  emit the type's scaffold headings after the legacy sections,
    filled from input when the type requires content, empty otherwise
  return id
#+end_example
...
section_input(input, heading):
  map a semantic heading to its create input value
...
epic appends empty Not yet specified and Decisions headings
task scaffold stays byte-identical to the legacy shape
#+end_example
