...
#+begin_src sh
TICKETS_DIR=/home/rosin/sys/.tickets mise exec -- cargo run --quiet -- migrate-legacy-properties
#+end_src

Review the report. Apply only when the diff is expected:

#+begin_src sh
TICKETS_DIR=/home/rosin/sys/.tickets mise exec -- cargo run --quiet -- migrate-legacy-properties --apply
#+end_src
